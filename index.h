...
.widget{
  box-shadow:0 60px 10px 5px rgba(0,0,0,0.4);
  margin:100px auto;
  height: 330px;
  position: relative;
  width: 500px;
...
function drawClock(){
    ...
    var today  = new Date();
    document.getElementById("date").innerHTML = today.toLocaleDateString("sk", options);
    ...
    /*
      Odošle HTTP request a získané JSON údaje rozparsuje a následne
      ich vloží do vytvoreného HTML dokumentu
    */  
    ...
    xhttp.onreadystatechange = function() {
      ...
      var obj = JSON.parse(txt); 
        ...
        document.getElementById("motion").innerHTML = String(obj.Motion);
       }  
      console.log(obj);
    };
   xhttp.open("GET", "readADC", true); // Riadi readADC server na ESP8266
   xhttp.send();
}
</script>
</body>
